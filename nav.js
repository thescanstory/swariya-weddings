...
function initImageActions() {
    const PINTEREST_PROFILE_URL = "https://in.pinterest.com/swariyaweddings/_profile/";
    const images = document.querySelectorAll('.gallery-grid img, .gallery-item img, .venue-card img, .hero-image img, .portfolio-grid img, .style-card img, .blog-card img, .real-wedding-card img, .decor-card img, .photo-grid img, .wedding-gallery img, section img');
    
    images.forEach(function (img) {
        // Skip small icons, logo, avatars, or QR codes
        if (img.src.includes('logo') || img.src.includes('icon') || img.src.includes('qr') || img.width < 50) return;
        ...
        pinBtn.title = 'View on Swariya Pinterest Board';
        pinBtn.href = PINTEREST_PROFILE_URL;
        pinBtn.innerHTML = `
        ...
        // Direct click on the image opens Swariya's Pinterest board in a new tab
        img.addEventListener('click', function (e) {
            // Don't intercept if parent is an explicit external or venue page link
            const parentAnchor = img.closest('a');
            if (parentAnchor && !parentAnchor.classList.contains('gallery-link') && !parentAnchor.classList.contains('pin-link')) {
                return;
            }
            e.preventDefault();
            e.stopPropagation();
            window.open(PINTEREST_PROFILE_URL, '_blank', 'noopener,noreferrer');
        });
    });
}
